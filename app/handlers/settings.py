...
from app.keyboards.inline import (
    settings_menu, bot_settings_kb, back_button,
...
async def cb_about(call: CallbackQuery, i18n: I18n, lang: str) -> None:
    ...
    kb = back_button(i18n, lang)
    await call.message.edit_text(text, reply_markup=kb)

@router.callback_query(F.data == "bot")
async def cb_bot(call: CallbackQuery, i18n: I18n, lang: str) -> None:
    text = i18n.get("settings.settings option", lang)
    uid = call.from_user.id
    cd = (await cache.json_get(await _cache_key(uid))) or {}
    cd.setdefault("update data", [])
    await cache.json_set(await _cache_key(uid), cd)
    await call.message.edit_text(str(text), reply_markup=bot_settings_kb(i18n, lang))
...
async def cb_fiat(call: CallbackQuery, i18n: I18n, lang: str) -> None:
    ...
    text = i18n.get("settings.output", lang)
    await call.message.edit_text(str(text),
...
async def cb_base_currency(call: CallbackQuery, i18n: I18n, lang: str) -> None:
    ...
    await call.message.edit_text(
        str(stx.get("base_currency_desc", "Select your default base currency:")),
    ...
    if command == "save":
        ...
        if "group" in cd:
            await db["Groups"].update_one(
                {"_id": cd["group"]}, {"$set": {prefix: data}})
        elif update_type:
            await db["Users"].update_one(
                {"_id": uid}, {"$set": {update_type: data}})
        await call.answer(stx.get("success", "Saved"), show_alert=False)
        cd["update data"] = []
        await cache.json_set(key, cd)
        await call.message.edit_text(
            str(stx.get("settings option", "Choose:")),
            reply_markup=bot_settings_kb(i18n, lang))
        return
